...
struct FluctuatingBlob: View {
    ...
    var body: some View {
        ZStack {
            ...
            Ellipse()
                .fill(Color.lsMint.opacity(0.80))
    ...
    var body: some View {
        Canvas { context, size in
            ...
            for i in 0...pts {
                ...
                // Break down the noise calculation into simpler sub-expressions
                let noise1 = sin(angle * 2 + phase * 0.7) * 0.12
                let noise2 = sin(angle * 3 - phase * 0.5) * 0.08
                let noise3 = sin(angle * 5 + phase * 0.9) * 0.05
                let noise = (noise1 + noise2 + noise3) * noiseAmp
            ...
            context.fill(blobPath, with: .color(Color.lsMint.opacity(0.9)))
