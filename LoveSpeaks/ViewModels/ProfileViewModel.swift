...
final class ProfileViewModel: ObservableObject {
    ...
    init() {
        currentDate = Self.formatDate(Date())
        Task { await boot() }
    }
    ...
    private func runSaludPadreModel(snapshot: HealthSnapshot) -> WellnessLabel {
        do {
            ...
            let model = try Profile_1(configuration: MLModelConfiguration())

            // ── Mapear estado del bebé a valores numéricos para el modelo ──
            let audioBabyState: String
            switch currentBabySound.category {
            case .quiet:
                audioBabyState = "calm"
            case .crying:
                audioBabyState = "crying"
            }

            // ── Estimar métricas de llanto basadas en el estado actual ──
            // En una implementación completa, esto vendría del AudioClassifierService
            // guardando un historial de eventos de llanto.
            let minutesSinceLastCry: Int64
            let nightCryingEpisodes: Int64
            let primaryCause: String
            let urgencyLevel: Int64

            if currentBabySound.category == .crying {
                minutesSinceLastCry = 0
                nightCryingEpisodes = 2  // Valor estimado
                primaryCause = "active_crying"
                urgencyLevel = Int64(currentBabySound.confidence * 5.0)  // 0-5 scale
            } else {
                // Baby is quiet
                minutesSinceLastCry = 120  // 2 horas desde el último llanto
                nightCryingEpisodes = 0
                primaryCause = "none"
                urgencyLevel = 1
            }

            // ── Pre-calcular valores con tipos explícitos ──
            let heartRateValue: Double = snapshot.heartRate > 0 ? snapshot.heartRate : snapshot.restingHR
            let restingHRValue: Double = snapshot.restingHR

            let hrvTrend: String = {
                let trend    = snapshot.weeklyHRV.last ?? snapshot.hrv
                let baseline = snapshot.hrv
                if trend > baseline * 1.10 { return "improving" }
                if trend < baseline * 0.90 { return "declining" }
                return "stable"
            }()

            let urgencyString: String = {
                switch Int(urgencyLevel) {
                case 0...1: return "low"
            ...
            // ── Construir el input completo del modelo ──
            let input = Profile_1Input(
                age:                     Int64(snapshot.age),
                sleep_hours:             snapshot.sleepHours,
                sleep_score:             Int64(snapshot.sleepScore),
                sleep_deep:              Double(snapshot.sleepDeep),
                sleep_core:              Double(snapshot.sleepCore),
                sleep_rem:               Double(snapshot.sleepREM),
                steps:                   Int64(Int(snapshot.steps)),
                heart_rate:              Int64(heartRateValue),
                resting_hr:              Double(restingHRValue),
                hrv:                     Double(snapshot.hrv),
                hrv_trend:               hrvTrend,
                respiratory_rate:        Double(snapshot.respiratoryRate),
                audio_baby_state:        audioBabyState,
                minutes_since_last_cry:  minutesSinceLastCry,
                night_crying_episodes:   nightCryingEpisodes,
                primary_cause:           primaryCause,
                urgency_level:           urgencyString
            )

            let output   = try model.prediction(input: input)
            // Create ML Tabular Classifier uses the target column name for output
            // Common names: "target", "state", or your custom column name
            let rawLabel = String(output.wellness_label)
    ...
    private func adjustForBabyState(_ label: WellnessLabel) -> WellnessLabel {
        let babyNeedsAttention = currentBabySound.category == .crying
    ...
    private func contextualInsight(for label: WellnessLabel) -> EmpatheticInsight {
        ...
        let babyCalm = currentBabySound.category == .quiet
